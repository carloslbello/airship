...
def init():
    try:
        ...
        global steamapi
        if system == 'Windows':
            steamapi = ctypes.CDLL(path + '/bin_win' + bits +
                                   '/CSteamworks.dll')
        elif system == 'Darwin':
            steamapi = ctypes.CDLL(path + '/bin_osx/CSteamworks.dylib')
        else:
            steamapi = ctypes.CDLL(path + '/bin_lnx' + bits +
                                   '/libCSteamworks.so')
        ...
        steamapi_get_file_name_size = \
            steamapi.ISteamRemoteStorage_GetFileNameAndSize
        ...
        steamapi_get_file_timestamp = \
            steamapi.ISteamRemoteStorage_GetFileTimestamp
        ...
        steamapi_file_write.argtypes = [ctypes.c_char_p, ctypes.c_char_p,
                                        ctypes.c_int]
        ...
        steamapi_cloud_enabled_account = \
            steamapi.ISteamRemoteStorage_IsCloudEnabledForAccount
        steamapi_cloud_enabled_account.restype = ctypes.c_bool
        global steamapi_cloud_enabled_app
        steamapi_cloud_enabled_app = \
            steamapi.ISteamRemoteStorage_IsCloudEnabledForApp
...
def will_work():
    return (steamapi_init() and steamapi_cloud_enabled_account() and
            steamapi_cloud_enabled_app())
...
def get_file_timestamp(filename):
    return steamapi_get_file_timestamp(((steamfolder + '/' if steamfolder
                                         else '') + filename).encode('utf-8'))


def read_file(filename):
    size = steamapi_get_file_size(((steamfolder + '/' if steamfolder else '') +
                                   filename).encode('utf-8'))
    stringbuffer = ctypes.create_string_buffer(size)
    steamapi_file_read(((steamfolder + '/' if steamfolder else '') + filename)
